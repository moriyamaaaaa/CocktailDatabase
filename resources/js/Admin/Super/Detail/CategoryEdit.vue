<template>
    <div id="container">
        ...
        <div v-if="category_id">
            <div>登録されているミドルカテゴリ一覧</div>
            <middle-list
                v-bind:is_display_narrow="false"
                v-bind:category_id="category_id"
            />
        </div>
        <hr>
    ...
    export default {
        ...
        methods: {
            async send() {
                ...
                //とりあえずデータは埋まっている
                try{
                    var result = await axios.post(process.env.MIX_SENTRY_DSN_PUBLIC+'/admin/submit/category',this.formdata, {headers: {'Content-Type': 'multipart/form-data'} })
                }catch(error){
                    console.log(error);
                }
